...
# ---------------------------------------------------------------------------
# 섹터 클러스터 — 로테이션 맵 그룹핑 (50+ ETF → 15개 클러스터)
# 패널에서는 열기점수(heat) 상위 6개만 자동 표시 — 지금 돈이 몰리는 섹터 동적 포착
# ---------------------------------------------------------------------------

SECTOR_CLUSTERS: dict[str, list[str]] = {
    # ── 기술 ──────────────────────────────────────────────────────────────
    "AI·반도체":        ["QQQ", "XLK", "SOXX", "SMH", "BOTZ", "QTUM",
                         "069500.KS", "229200.KS", "396500.KS", "139260.KS",
                         "381180.KS", "456600.KS"],
    ...
    "원전·우라늄":       ["URA", "434730.KS", "433500.KS"],
    "원유·에너지":       ["XLE", "USO", "139250.KS"],

    # ── 방산·항공우주 ──────────────────────────────────────────────────────
    "방산·우주":         ["SHLD", "ITA", "UFO", "449450.KS"],

    # ── 한국 특화 ──────────────────────────────────────────────────────────
    "조선·K방산":        ["494670.KS", "449450.KS"],
    "2차전지·EV":        ["LIT", "305540.KS"],
    "K뷰티·콘텐츠":      ["266360.KS", "479850.KS", "228790.KS"],

    # ── 헬스·소비 ──────────────────────────────────────────────────────────
    "바이오·헬스":       ["XLV", "IBB", "244580.KS", "143860.KS"],
    "소비재·경기소비":   ["XLY", "XLP"],

    # ── 금융·산업재 ────────────────────────────────────────────────────────
    "금융·핀테크":       ["XLF", "FINX", "139270.KS"],
    "산업재·원자재":     ["XLI", "XLB", "COPX", "XHB", "139220.KS"],

    # ── 글로벌·안전자산 ────────────────────────────────────────────────────
    "신흥국":            ["FLIN", "EWZ", "EWJ", "EWT", "MCHI",
                         "453870.KS", "192090.KS", "241180.KS"],
    "안전자산·채권":     ["TLT", "IAU", "SLV", "SHV", "UUP", "HYG"],
}
