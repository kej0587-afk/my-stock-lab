...
def apply_safety_state_override(
    decision_outcome,
    dec,
    col,
    *,
    ...
    """Downgrade aggressive new-entry signals when the safety state is RED."""
...
def translate_new_entry_decision_for_holding(decision_outcome, *, has_pos, weight_gap=0.0):
...
def has_down_session_pressure(
    *,
    day_ret,
    regular_day_ret=math.nan,
    live_ref_ret=math.nan,
    live_gap_move=math.nan,
...
def build_sideways_quality_state(c: dict, *, is_leveraged_product=False, has_pos=False) -> dict:
    """Translate timing inputs into an easy sideways/pullback quality label."""
    ...
    rsi = clean_float(c.get("rsi"), math.nan)
    mfi = clean_float(c.get("mfi"), math.nan)
    ...
    day_ret = clean_float(c.get("day_ret"), math.nan)
    regular_day_ret = clean_float(c.get("regular_day_ret"), math.nan)
    live_ref_ret = clean_float(c.get("live_ref_ret"), math.nan)
    live_gap_move = clean_float(c.get("leveraged_drop_ret", c.get("day_ret")), math.nan)
    vol_ratio = clean_float(c.get("vol_ratio"), math.nan)
    ...
    ma20_gap = (cur / ma20 - 1.0) if ma20 > 0 else math.nan
    ma5_gap = (cur / ma5 - 1.0) if ma5 > 0 else math.nan
    ...
    down_pressure = has_down_session_pressure(
        day_ret=day_ret,
...
def compute_sizing_hint(
    ...
    *,
    ...
    if (
        ...
        and targ_w > 0
        and weight_gap >= 1.0
        and decision_outcome.code in (
            "LEADER_MA5_PULLBACK_ENTRY",
            "LEADER_MA5_FAST_PULLBACK_ENTRY",
            "OVERHEAT_EXTENSION_WAIT_MA5",
            "S_GRADE_OVERHEAT_WAIT",
        )
    ):
