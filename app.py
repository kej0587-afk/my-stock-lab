...
# 1. 기본 설정
# -------------------------------------------------
st.set_page_config(page_title="대장님의 최종 관제실 v7.6 (보안망 적용)", layout="wide")
...
st.title("🚀 REALTIME DIGITAL DASHBOARD v7.6")

# -------------------------------------------------
# 2. 구글 보안 인증 및 데이터 로드 (핵심 변경점)
# -------------------------------------------------
...
def load_portfolio_sheet():
    ...
    df = pd.DataFrame({
        "통화": data.iloc[:, 0], "자산클래스": data.iloc[:, 1], "국가": data.iloc[:, 2], "자산구분": data.iloc[:, 3],
        "자산명": data.iloc[:, 4], "요약": data.iloc[:, 5], "티커입력": data.iloc[:, 6], "보유량": data.iloc[:, 7],
        "매입가": data.iloc[:, 8], "현재가": data.iloc[:, 9], "수익률": data.iloc[:, 10], "평가손익": data.iloc[:, 11],
        "평가금액": data.iloc[:, 12], "원화환산": data.iloc[:, 13],
    })
    for col in ["통화", "자산클래스", "국가", "자산구분", "자산명", "요약", "티커입력"]: df[col] = df[col].astype(str).str.strip()
    for col in ["보유량", "매입가", "현재가", "수익률", "평가손익", "평가금액", "원화환산"]: df[col] = df[col].apply(parse_num).fillna(0)
    return df
...
def calc_scores_and_decision(name, ticker, is_etf, asset_class, df, my_price, fin_score):
    ...
    if is_etf:
        if (current_w > target_w and target_w > 0): dec, col = "🛑비중 초과: 추가매수 금지", "#dc2626"
...
def get_all_summary():
    ...
    for name, (tkr, is_etf, asset_class) in TICKER_MAP.items():
        ...
        calc = calc_scores_and_decision(name, tkr, is_etf, asset_class, df, my_price, 0 if is_etf else 2)
        rows.append({
            "종목명": name, "현재가": format_currency(calc["cur_p"], tkr), "현재비중": f'{calc["current_w"]:.2f}%', "목표비중": f'{calc["target_w"]:.2f}%',
            ...
            "SQZ": calc["sqz_status"], "🔥기술적 타점": calc["dec"], "Adj점수": round(calc["adj_tech_score"], 1)
        })
...
with tab2:
    ...
    if not df.empty:
        df = build_indicators(df)
        calc = calc_scores_and_decision(sel_name, sel_ticker, is_etf, asset_class, df, my_price, fin_score)
        last = df.iloc[-1]
        c1, c2 = st.columns([1.2, 2.3])

        with c1:
            st.markdown(f"<h2>📊 {sel_name}</h2>", unsafe_allow_html=True)
            st.markdown(f"<div class='info-panel'>현재가: <span class='highlight'>{format_currency(calc['cur_p'], sel_ticker)}</span></div>", unsafe_allow_html=True)
