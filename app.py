...
def calc_scores_and_decision(name, ticker, is_etf, asset_class, df, my_price, has_pos, fin_score, is_free_search=False):
    ...
    is_early_entry = (
        ...
        macd_state in ["📉하락주의(데드크로스)", "⏳추세관망"] and
        mfi_now < 80 and
        pct_b_now < 0.85 and
        50 <= rsi_now <= 65 and
        adj_tech_score >= 4.0
    ...
    if rsi_now <= 30:
        ...
        smc_insight = "스마트머니 익절 가능성이 높은 단기 과열 구간."
    elif 0.45 < pct_b_now < 0.8 and sqz_status == "🚀해제직후":
        smc_insight = "응축 후 발산 초기. 모멘텀 실리는 타점 구간."
    ...
    if is_free_search:
        ...
        elif is_breakout_buy_extreme:
            dec, col = "⚠️과열확장: 추격금지, MA5 복귀 대기", "#d97706"
        elif is_breakout_buy_normal:
            dec, col = "🔥불뿜는 대장주: 초단기 눌림(MA5) 진입 검토", "#ec4899"
        elif pct_b_now >= 0.95:
            dec, col = "⚠️밴드상단: 눌림 대기", "#d97706"
        elif current_dd <= -0.2:
            dec, col = "🚨위기/패닉: 투매 포착(분할접근)", "#dc2626"
        ...
        elif ret_3m < 0 and trend_label in ["🌊역배열(하락)", "⏳혼조세"]:
            dec, col = "⚠️하락추세: 진입보류", "#dc2626"
        elif trend_label == "🌊역배열(하락)":
            dec, col = "🚫역배열: 진입 보류", "#dc2626"
        else:
        ...
        if (not is_etf) and fin_score == 1:
            ...
            dec, col = "💣패닉(-50%↓): 나스닥100% 보너스 30% 최종투입", "#7f1d1d"
        elif current_dd <= -0.4:
            dec, col = "💣패닉(-40%↓): 나스닥100% 보너스 40% 투입", "#991b1b"
        elif current_dd <= -0.3:
            dec, col = "🚨위기(-30%↓): 나스닥 60% SP 40% / 코어ETF 100% 집중", "#b91c1c"
        elif current_dd <= -0.2:
            dec, col = "🚨위기(-20%↓): 현금30% 확보 및 코어 집중", "#dc2626"
            ...
            elif trend_label == "🚀정배열(상승)" and pct_b_now < 0.8 and rsi_now < 60 and cur_p <= my_price:
                dec, col = "📈정배열: 눌림목 매수", "#16a34a"
            elif cur_p > my_price:
...
def get_all_summary(fin_score_map_items):
    ...
    for name, (tkr, is_etf, a_class) in TICKER_MAP.items():
        ...
        rows.append({
            ...
            "MDD": f"{c['dd']*100:.1f}%",
            "현재비중": f"{c['current_w']:.2f}%",
            "목표비중": f"{c['target_w']:.2f}%",
            "📌후보등급": c["grade"],
            "RS": c["rs_label"],
            "RSI": round(c["rsi"], 1),
            "MFI": round(c["mfi"], 1),
...
with tab1:
    ...
    st.dataframe(
        get_all_summary(tuple(sorted(st.session_state.fin_score_map.items()))),
