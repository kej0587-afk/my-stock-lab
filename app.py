...
상대적으로 약한 쪽은 <b>{weak['하위테마']}</b>입니다.
    ...
    with summary_tab:
        ...
        for col in ["가격수준", "1개월수익률", "3개월수익률", "6개월수익률", "가속도"]:
            show_group[col] = show_group[col].apply(fmt_flow_pct)
        show_group["돈흐름점수"] = show_group["돈흐름점수"].map(lambda x: f"{x:.1f}")
        st.dataframe(
            show_group[["하위테마", "종목수", "대표주", "1개월수익률", "3개월수익률", "6개월수익률", "가속도", "가격수준", "돈흐름점수", "상태", "구성종목"]],
            use_container_width=True,
            hide_index=True,
            height=430,
        )
    ...
    with stocks_tab:
        ...
        st.dataframe(
            show_stock[["하위테마", "종목명", "Ticker", "현재가", "가격수준", "1개월수익률", "3개월수익률", "6개월수익률", "가속도", "돈흐름점수", "상태"]],
            use_container_width=True,
            hide_index=True,
            height=560,
        )
...
# ---------------------------------------------------------
# [최종본] 인쇄용 리포트 렌더링 함수 및 스위치 (app.py 맨 아래 배치)
# ---------------------------------------------------------
def render_full_print_report():
    ...
    <table style="width:100%; text-align:center; font-size:1.1em; border-collapse:collapse; margin-bottom:20px;">
        <tr style="background-color:#1e293b; color:white;">
            <th style="padding:10px; border:1px solid #334155;">총자산</th>
            <th style="padding:10px; border:1px solid #334155;">투자자산</th>
            <th style="padding:10px; border:1px solid #334155;">누적수익률</th>
            <th style="padding:10px; border:1px solid #334155;">누적손익</th>
        </tr>
        <tr style="background-color:#0f172a; color:white;">
            <td style="padding:15px; border:1px solid #334155; font-weight:bold;">{portfolio_summary.get('current_asset', 0):,.0f}원</td>
            <td style="padding:15px; border:1px solid #334155; font-weight:bold;">{portfolio_summary.get('stock_value', 0):,.0f}원</td>
            <td style="padding:15px; border:1px solid #334155; font-weight:bold; color:{'#22c55e' if portfolio_summary.get('cum_return', 0) >= 0 else '#ef4444'};">{portfolio_summary.get('cum_return', 0):.2f}%</td>
            <td style="padding:15px; border:1px solid #334155; font-weight:bold; color:{'#22c55e' if portfolio_summary.get('cum_profit', 0) >= 0 else '#ef4444'};">{portfolio_summary.get('cum_profit', 0):,.0f}원</td>
        </tr>
    </table>
    """, unsafe_allow_html=True)
    ...
    if not dash_df.empty:
        ...
        with hc1:
            ...
            fig_tree.update_layout(template="plotly_dark", height=320, margin=dict(t=30, l=10, r=10, b=10), title="포트폴리오 히트맵")
            st.plotly_chart(fig_tree, use_container_width=True)
        with hc2:
            max_eval = max(float(dash_df["원화환산"].max() or 0), 1.0)
            bubble_size = np.clip(np.sqrt(dash_df["원화환산"] / max_eval) * 45, 10, 45)
            fig_bubble = go.Figure(go.Scatter(
        ...
        with wc1:
            fig_weight = go.Figure()
            fig_weight.add_trace(go.Bar(y=dash_df["자산명"], x=dash_df["현재비중"], orientation="h", name="현재비중"))
            fig_weight.add_trace(go.Bar(y=dash_df["자산명"], x=dash_df["리밸런싱목표비중"], orientation="h", name="목표비중"))
            fig_weight.update_layout(template="plotly_dark", barmode="group", height=320, margin=dict(t=10, l=10, r=10, b=10))
            st.plotly_chart(fig_weight, use_container_width=True)
        with wc2:
            pnl_color = np.where(dash_df["평가손익_원화"] >= 0, "#16a34a", "#dc2626")
    ...
    with st.spinner("포트폴리오 위험 지표 계산 중..."):
        ...
        <table style="width:100%; text-align:center; font-size:1.1em; border-collapse:collapse; margin-bottom:20px;">
            <tr style="background-color:#1e293b; color:white;">
                <th style="padding:10px; border:1px solid #334155;">위험도 등급</th>
                <th style="padding:10px; border:1px solid #334155;">연환산 변동성</th>
                <th style="padding:10px; border:1px solid #334155;">1년 MDD</th>
                <th style="padding:10px; border:1px solid #334155;">평균 상관계수</th>
            </tr>
            <tr style="background-color:#0f172a; color:white;">
                <td style="padding:10px; border:1px solid #334155; font-weight:bold; color:{metrics.get('risk_color', 'white')};">{metrics.get('risk_grade', '-')} ({metrics.get('risk_index', 0):.0f}점)</td>
                <td style="padding:10px; border:1px solid #334155; font-weight:bold;">{metrics.get('portfolio_vol', 0):.1f}%</td>
                <td style="padding:10px; border:1px solid #334155; font-weight:bold;">{metrics.get('portfolio_mdd', 0):.1f}%</td>
                <td style="padding:10px; border:1px solid #334155; font-weight:bold;">{metrics.get('avg_corr', 0):.2f}</td>
            </tr>
        </table>
        """, unsafe_allow_html=True)
    ...
    # ==========================================
    st.header("4. 핵심 종목 타점 및 대표 종목 정밀 분석")
    st.markdown("#### 🔸 관심/보유 종목 요약 (전광판)")
    summary_df = get_all_summary(tuple(sorted(st.session_state.fin_score_map.items())), app_mode, tuple(st.session_state.watchlist))
    ...
    st.markdown("---")
    st.markdown("#### 🔍 정밀 관측소 분석 예시: 나스닥 100 (QQQM)")
    ...
    if not ex_df.empty:
        ...
        st.markdown(f"""
        <table style="width:100%; text-align:center; font-size:1.1em; border-collapse:collapse; margin-bottom:10px;">
            <tr style="background-color:#1e293b; color:white;">
                <th style="padding:10px; border:1px solid #334155;">현재가</th>
                <th style="padding:10px; border:1px solid #334155;">최종 판정</th>
                <th style="padding:10px; border:1px solid #334155;">후보 등급 (Adj 점수)</th>
            </tr>
            <tr style="background-color:#0f172a; color:white;">
                <td style="padding:10px; border:1px solid #334155; font-weight:bold;">{format_currency(c['cur_p'], ex_ticker)}</td>
                <td style="padding:10px; border:1px solid #334155; font-weight:bold; color:{c['col']};">{c['dec']}</td>
                <td style="padding:10px; border:1px solid #334155; font-weight:bold;">{c['grade']} ({c['adj']:.1f}점)</td>
            </tr>
        </table>
        """, unsafe_allow_html=True)
